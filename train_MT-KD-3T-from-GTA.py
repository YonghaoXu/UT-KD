...
def main():
    ...
    data_list_target1 = './dataset/cityscapes_labellist_train.txt'
    data_list_val1 = './dataset/cityscapes_labellist_val.txt'
    data_dir_target1 = '/iarai/home/yonghao.xu/Data/SegmentationData/Cityscapes/'     
    input_size_test1 = (2048,1024)
    ...
    tgt1_loader = data.DataLoader(
                    cityscapesDataSet(data_dir_target1, data_list_target1, max_iters=args.num_steps_stop*args.batch_size,                  
    ...
    val1_loader = data.DataLoader(
                    cityscapesDataSet(data_dir_target1, data_list_val1, max_iters=None,                  
                    crop_size=input_size_tgt,
                    scale=False, mirror=False, mean=IMG_MEAN,
    ...
    data_dir_target2 = '/iarai/home/yonghao.xu/Data/SegmentationData/IDD/IDD_Segmentation/'
    data_list_val2 = './dataset/IDD_val.txt'
    data_list_target2 = './dataset/IDD_train.txt'
    input_size_test2 = (1920,1080)

    # G->C&I&M      
    snapshot_dir = args.snapshot_dir+'synthetic2real_GTA2City_IDD_Mapillary/MTKD_out_weight_'+str(args.out_weight)+'_con_weight_'+str(args.con_weight)+'_time'+time.strftime('%m%d_%H%M', time.localtime(time.time()))+'/'
    tgt2_loader = data.DataLoader(
                IDDDataSet(data_dir_target2, data_list_target2, max_iters=args.num_steps_stop*args.batch_size,                  
                crop_size=input_size_tgt,
                scale=False, mirror=False, mean=IMG_MEAN,
                set='train'),
                batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers,
    ...
    val2_loader = data.DataLoader(
                IDDDataSet(data_dir_target2, data_list_val2, max_iters=None,                  
                crop_size=input_size_tgt,
                scale=False, mirror=False, mean=IMG_MEAN,
                set='val'),
                batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers,
    ...
    data_dir_target3 = '/iarai/home/yonghao.xu/Data/SegmentationData/MapillaryVistas/'
    data_list_val3 = './dataset/Mapillary_val.txt'
    data_list_target3 = './dataset/Mapillary_train.txt'
    input_size_test3 = (1920,1080)

    tgt3_loader = data.DataLoader(
                MapDataSet(data_dir_target3, data_list_target3, max_iters=args.num_steps_stop*args.batch_size,                  
                crop_size=input_size_tgt,
                scale=False, mirror=False, mean=IMG_MEAN,
                set='train'),
                batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers,
    ...
    val3_loader = data.DataLoader(
                MapDataSet(data_dir_target3, data_list_val3, max_iters=None,                  
                crop_size=input_size_tgt,
                scale=False, mirror=False, mean=IMG_MEAN,
                set='val'),
                batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers,
    ...
    for batch_index, (src_data, tgt1_data, tgt2_data, tgt3_data) in enumerate(zip(src_loader, tgt1_loader, tgt2_loader, tgt3_loader)):
        ...
        tgt3_image, _,_,_ = tgt3_data
        tgt3_image = tgt3_image.cuda()
        tgt3_2src = trans_net(tgt3_image,customizedin_gta)  
        tgt3_2src_1 = tgt3_2src[:half_batch,:,:,:]
        images3_1 = tgt3_image[:half_batch,:,:,:]
        images3_2 = tgt3_image[half_batch:,:,:,:]
        
        mask_size = tgt3_2src_1.shape[2:]
        # (N,1,H,W)
        batch_mix_masks = torch.from_numpy(mask_generator.generate_params(len(tgt3_2src_1), mask_size)).cuda().float()
            
        # Mix images with masks
        mixed3_s = tgt3_2src_1 * (1 - batch_mix_masks) + images3_2 * batch_mix_masks
       
        _,tgt3_s_output_mixed_s = student_net(mixed3_s)
        _,tgt3_t_output_ori1 = teacher_net(images3_1)
        _,tgt3_t_output_ori2 = teacher_net(images3_2)

        tgt3_t_output = interp_tgt(tgt3_t_output_ori1) * (1 - batch_mix_masks) + interp_tgt(tgt3_t_output_ori2) * batch_mix_masks
        tgt3_s_output = interp_tgt(tgt3_s_output_mixed_s)
        
        tgt3_t_predicts = F.softmax(tgt3_t_output, dim=1).transpose(1, 2).transpose(2, 3)
        tgt3_s_predicts = F.softmax(tgt3_s_output, dim=1).transpose(1, 2).transpose(2, 3)

        tgt3_s_predicts = tgt3_s_predicts.contiguous().view(-1,n_class)
        tgt3_t_predicts = tgt3_t_predicts.contiguous().view(-1,n_class)

        con_loss_value = con_loss(tgt1_s_predicts, tgt1_t_predicts) + con_loss(tgt2_s_predicts, tgt2_t_predicts) \
                         + con_loss(tgt3_s_predicts, tgt3_t_predicts)

        tgt3_s_output_ori1,tgt3_s_output_ori2 = student_net(tgt3_image)
        
        tgt3_s_output1 = interp_tgt(tgt3_s_output_ori1)
        tgt3_s_output2 = interp_tgt(tgt3_s_output_ori2)
        ...
        tgt3_D_out1 = D_net1(F.softmax(tgt3_s_output1, dim=1))
        tgt3_D_out2 = D_net2(F.softmax(tgt3_s_output2, dim=1))
        ...
        tgt_adv_loss_1 = bce_loss(tgt1_D_out1, Variable(torch.FloatTensor(tgt1_D_out1.data.size()).fill_(source_label)).cuda()) \
                      ...
                      + bce_loss(tgt3_D_out1, Variable(torch.FloatTensor(tgt3_D_out1.data.size()).fill_(source_label)).cuda())
        ...
        tgt_adv_loss_2 = bce_loss(tgt1_D_out2, Variable(torch.FloatTensor(tgt1_D_out2.data.size()).fill_(source_label)).cuda()) \
                      ...
                      + bce_loss(tgt3_D_out2, Variable(torch.FloatTensor(tgt3_D_out2.data.size()).fill_(source_label)).cuda())
        ...
        tgt3_pred1 = tgt3_s_output1.detach()
        tgt3_pred2 = tgt3_s_output1.detach()
        ...
        tgt3_D_out1 = D_net1(F.softmax(tgt3_pred1, dim=1))
        tgt3_D_out2 = D_net2(F.softmax(tgt3_pred2, dim=1))
        ...
        loss_D1 = bce_loss(tgt1_D_out1, Variable(torch.FloatTensor(tgt1_D_out1.data.size()).fill_(target_label)).cuda())*0.5 \
                ...
                + bce_loss(tgt3_D_out1, Variable(torch.FloatTensor(tgt3_D_out1.data.size()).fill_(target_label)).cuda())*0.5
        ...
        loss_D2 = bce_loss(tgt1_D_out2, Variable(torch.FloatTensor(tgt1_D_out2.data.size()).fill_(target_label)).cuda())*0.5 \
                ...
                + bce_loss(tgt3_D_out2, Variable(torch.FloatTensor(tgt3_D_out2.data.size()).fill_(target_label)).cuda())*0.5
        ...
        if (batch_index+1) % 500 == 0:                    
            ...
            mIoU_val3 = test_mIoU7(f,teacher_net, val3_loader, batch_index+1,input_size_test3,print_per_batches=10)
            
            # Saving the models        
            if mIoU_new > mIoU_hist:    
                f.write('Save Model\n') 
                print('Save Model')                     
                model_name = 'Seg_batch_'+repr(batch_index+1)+'_miou_'+repr(int(mIoU_new*100))+'_val2_miou_'+repr(int(mIoU_val2*100))+'_val3_miou_'+repr(int(mIoU_val3*100))+'.pth'
