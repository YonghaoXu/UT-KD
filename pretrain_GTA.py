...
def get_arguments():
    ...
    parser.add_argument("--restore_from", type=str, default='./pretrained_model/DeepLab_resnet_pretrained_init-f81d91e8.pth',
...
def main():
    ...
    snapshot_dir = args.snapshot_dir+'GTA_time'+time.strftime('%m%d_%H%M', time.localtime(time.time()))+'/'
    ...
    # Create network
    base_net = DeeplabMulti(num_classes=args.num_classes)
    saved_state_dict = torch.load(args.restore_from)

    new_params = base_net.state_dict().copy()
    for i in saved_state_dict:
        # Scale.layer5.conv2d_list.3.weight
        i_parts = i.split('.')
        # print i_parts
        if not args.num_classes == 7 or not i_parts[1] == 'layer5':
            new_params['.'.join(i_parts[1:])] = saved_state_dict[i]
            # print i_parts
    base_net.load_state_dict(new_params)
    ...
    src_loader = data.DataLoader(
                    ...
                    scale=False, mirror=False, mean=IMG_MEAN,num_classes=7),
                    batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True)

    optimizer = optim.SGD(base_net.optim_parameters(args),
    ...
    for batch_index, src_data in enumerate(src_loader):
        ...
        _,src_output_ori = base_net(images)
        ...
        if (batch_index+1) % 10000 == 0: 
            ...
            f.write('Save Model\n') 
            print('Save Model\n')                     
            model_name = 'GTA_pretrain_batch_'+repr(batch_index+1)+'_miou_'+repr(int(np.mean(loss_hist[batch_index-9:batch_index+1,1])*100))+'.pth'
            torch.save(base_net.state_dict(), os.path.join(
